...
* RMD allows us to automatically create __formats that are easier to present and read__ than raw code.  
* RMD lets us craft *several* different kinds of outputs.  
* RMD can read and interpret __multiple languages__.  
* RMD makes collaboration easy with *clean* code and inserting notes and explanations. This is also useful for teaching.
...
#2 columns total, 1 w dataset names, 2nd with a description
#manually type in table items
#include 4 example tables
...
```{r installing, eval = FALSE, include = FALSE}
...
Nutrients <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv", header = TRUE)
