...
#this is the tidied up data that we created in Part 1 of lesson 4
NTL.nutrient.data <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv",stringsAsFactors = TRUE)
#notice this data has a new category for depth: depth_id
```

## Review of basic exploration and wrangling
```{r}
#want to check parameters on your data even if you've already wrangled and processed it
...
# Format sampledate as date - now we have year w 4 digits so use Y
...
# Notice that other lake names didn't go away, even though they have zero values - selected a subset and that data is shown, but the other names were kept but none of their data was. use fxn below to get rid of them
...
#pipe method to perform operations on dataset:
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #beginning of pipe
  mutate(month = month(sampledate)) %>% #mutate to make a month only col
  select(lakeid:daynum, month, sampledate:comments) %>% #reorder cols 
  drop_na(depth) #all NAs in data are being dropped

# Save processed nutrient file outside of R
...
#we removed lakeid for both datasets

# write a more succinct line of code to subset the nutrient dataset. 
#select(NTL.nutrient.data.PeterPaul, lakename:daynym, sampledate:po4)
```
...
#above: transform 4 cols into 2: one w category for nutrients (colnames) and one with values 
#tn_ug:po4 columns should be spread into 2 columns, one called nutrients that has the names and one called concentration that has values
...
#lots of NAs in the concentration values so want to get rid of those NAs - this will lower the number of rows in the dataframe
#count above shows you how many concentrations we have for each nutrient
...
#store processed dataset to a new file - do this with every major step
...
#above: basically opp of what we did with pivot_longer. note this has 2 fewer rows than the dataset we used to make the pivot_longer dataframe. likely bc of our removal of NAs
...
#tell dataframe which column you want to separate and then provide the format to be separated into (bc rn the date is in %Y%-%m%-%d%)
#unite does the opposite of separate - bring together multiple columns into 1
...
LitterFiles
#above imports all csv files within the folder indicated in the path

Litter <- LitterFiles %>%
  ldply(read.csv)
#used ldply to create a new dataframe that is made up of all csv files in LitterFiles list of files by binding them via rows
```
...
#think about which columns we need for analysis. we select these above
...
NTL.PeterPaul.summaries <- 
  NTL.phys.nutrient.data.PeterPaul %>% #choose data
  filter(depth == 0) %>% #filter by depth
  group_by(lakename, month) %>% #group by lake name and month
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #filter out NAs - only choosing rows where all 3 of these columns don't have NAs
  summarise(meantemp = mean(temperature_C), 
            sdtemp = sd(temperature_C), 
            meanTN = mean(tn_ug), 
            sdTN = sd(tn_ug), 
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug)) #telling R what we want from cleaned up data
