...
#Set the URL - this directs us to the web page we pull data from but after right clicking to view page source rather than the normal website.
...
#from web address above, data we want starts on the 30th line. \t means tab.

#Update the column headers - these are on line 29 of above URL
colnames(gage_data) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data - convert to date.
gage_data <- gage_data %>% 
...
### Exercise: 
> Alter the above code so that it plots the last 24 days of flow for gage 02085070 - these are 2 quick changes in the URL and that's it. site_no needs to be changed in URL and period needs to be changed in URL
...
#if we know the structure of the URL and what we need to change to move bw sites etc, we can then use paste and create a for loop to gather data from diff sites
...
#Identify gauge to download (from NWIS site numbers above)
siteNo <- '02087500' 

#Identify parameter of interest (discharge amt): 
pcode = '00060' #discharge (cfs)

#Identify statistic code for daily values (mean daily value): 
scode = "00003"  #mean
...
#the readNWIS commands are part of the data retrieval package. 
...
#a lot of APIs require a key. you can request a key (they're free) at above key_signup website. requesting keys is a way for census bureau to track how many keys people are asking for etc. can't do below code until you get a key and insert it in code above.
 ...
 * Check for terms of service or robots.txt <- can't find any on eia.gov
