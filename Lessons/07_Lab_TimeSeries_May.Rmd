...
#if we look at Statistics column it says "Jan Average" etc so we need to separate these
...
#now we just have rainfall and date data in this new dataframe
...
#above df has just temp and date data
...
#now we have date, rainfall, and temp in our dataset
...
#if short interval: use linear interpolation to fill gaps.

summary(BRA_complete)
#no NAs so looks like we aren't missing data
...
#temp has upward trend, rainfall doesn't have much of a trend.
...
f_date <- first(BRA_complete$Date)

BRArain.ts <- ts(BRA_complete$Rainfall, start =f_date, 
             frequency = 12)

BRAtemp.ts <- ts(BRA_complete$Temperature_C, start =f_date, 
             frequency = 12)
...
#when the dataset has no seasonality, you don't use periodic, say frequency = 1
BRArain.decomp <- stl(BRArain.ts, s.window = "periodic")

BRAtemp.decomp <- stl(BRAtemp.ts, s.window = "periodic")

plot(BRArain.decomp)

plot(BRAtemp.decomp)

#seasonal components to both datasets
#increasing trend in temp
#somewhat stationary for rain
...
BRAtemps.comps <- as.data.frame(BRAtemp.decomp$time.series[,1:3])

BRArain.comps <- as.data.frame(BRArain.decomp$time.series[,1:3])



# Exercise 10: Visualize how the trend maps onto the data for both series
BRAtemps.comps <- mutate(BRAtemps.comps,
        Observed = BRA_complete$Temperature_C,     
        Date = BRA_complete$Date)

BRArain.comps <- mutate(BRArain.comps,
        Observed = BRA_complete$Rainfall,     
        Date = BRA_complete$Date)

ggplot(BRAtemps.comps) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "orchid2") +
  #geom_hline(yintercept = 0, lty = 2) +
  ylab(expression(paste("Temperature, ", degree, "C"))) +
  ylim(c(20,30))

ggplot(BRArain.comps) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
...
#statistically proving there is a trend using Seasonal M-K:
...
#use just M-K after removing seasonality from time series dataframe:
