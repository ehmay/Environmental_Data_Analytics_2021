...
webpage
#data on website is water use reports, water withdrawals and transfers
#telling R go to a website and go to its contents
#this page is a set of nodes and a set of documents
```
...
```{r scrape.some.data}
#new variable that's extracted from the webpage via the nodes that include the tag we got from the selector tool:
...
#can select multiple items by keeping more than 1 thing highlighted on the webpage with the gadget tool
...
#once we have data, use wrangling skills to make a df
#Create a dataframe of withdrawals
df_withdrawals <- data.frame("Month" = rep(1:12),#list 1-12
                             "Year" = rep(2000,12), #repeat 2000 12x
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals),#change from chr to num
...
#!! = "bang" = "don't set it to this as a string, set it to the variable named this"
#Plot 
ggplot(df_withdrawals,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  ...
  labs(title = paste("2020 Water usage data for",the_registrant),
       ...
       y="Withdrawal (mgd)",
       x="Date")
#plot shows our scraped data (in black, a line plot) and an interpolated trend line (loess)
```
...
#breaking URL into components
#paste0 eliminates adding a space bw elements
...
#so now we've built the site by pasting its components together

#Retrieve the website contents to create a hook into the website:
the_website <- read_html(the_scrape_url)

#Set the element address variables (determined in the previous step) [converting input tags into variables]
...
#instead of overriding above code to create a new df for each site/year, what if we want to build on existing df?
```
...
#Create our scraping function with 2 inputs, year and facility
...
#iterating through this fxn lets us scrape multiple datasets

#sys.sleep(1) tells R to wait for (#) of seconds. we do this to run our code more slowly - important when we're iterating a lot of times and asking a lot of stuff of the server over and over bc R will worry that you're doing something fishy pulling data from a website and will alert site provider, who may cut off access. dw about it if it's just a couple sites, as below.
```
...
the_years = rep(2018:2020) #this doesn't show up in my env either?
my_facility = '0004-0001'

#Use lapply to apply the scrape function. running it for every value in the the_years vector
...
#bind_rows is tidyverse version of rbind
...
ggplot(the_df,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  ...
  labs(title = paste("2020 Water usage data for",the_registrant),
       ...
       y="Withdrawal (mgd)",
       x="Date")
#above code for labeling is wrong - John will fix
```
...
#example: getting facility numbers without having to individually click on all the links for all the individual sites
#we can grab links from web pages like we grabbed datasets above
#crawling: start w web page, crawl in deeper to get links etc
#Set the web page
...
#Use Selector Gadget to find the tag for all the "View Report" objects, they are all linked on main webpage
#You should find this to be "#content a"

#Then extract all the nodes associated with this and rather than extracting them. don't use html_text bc this will just print the text and not the links and their info 
the_facility_ids <- the_main_website %>% 
  ...
  map_chr(.,9)                    #Extract the 9th item in the above [the webpage for a specific site], which is the facility ID
...
#Subset the facilities: 20 at random
the_facility_ids_subset <-  sample(the_facility_ids,20)

#Create a list of the year we want, the same length as the vector above: list of years matching length of facility IDs
the_years <- rep.int(2020,length(the_facility_ids_subset))
#doing the above bc we're iterating for 2 items

#"Map" the "scrape.it" function to retrieve data for all these
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) 

#map2 iterates through the 2 lists in parallel: take 1st item in each pair and run that, then 2nd item in each pair and run that, etc until the last item.

#Conflate the returned list of dataframes into a single one
df_2020 <- bind_rows(dfs_2020)
#binding above list of dfs into a single df
...
#here, if we were to use map2 it would only give 2018 for site 1, 2019 for site 2, 2020 for site 3. but we want all 3 years for each site.
...
the_df <- cross2(the_years,the_facilities) %>% 
  map(lift(scrape.it)) %>% 
  bind_rows()
#above: taking list we created and applying it into the scrape.it fxn we made, then binding things together
...
#my graphs looked different from his bc the sites that are pulled are RANDOM. so dw about not matching video graphs.
