...
> Add notes here: Want to look at health of the ecosystems and water quality for health implications.
...
#chemistry and physics data in this file
#using a relative path for reproducability
#stringAsFactors lets summary function run on characters and actually give useful outputs
colnames(NTL.phys.data)
head(NTL.phys.data)
summary(NTL.phys.data)
str(NTL.phys.data)
#sampledate is a factor rn (bc it was chr and we converted it - see below for transforming it into date)
dim(NTL.phys.data)
#we see multiple rows w same sample date bc we have diff values of things like oxygen at different depths on the same day

class(NTL.phys.data$sampledate)
# Format sampledate as date
#we have yy not YYYY so we use %y below
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y")
#note that the whole year, e.g. 1984, shows up when we convert data to dates. the %y is just so R recognizes the original date form, but it will always then put the data in its full yyyy-mm-dd form upon conversion.
```
...
```{r, results = "hide"}
vignette("dplyr")
#mutate is useful for creating a new column based on existing columns' data
```
...
`&`
and - aggregate multiple conditions
`|`
...
#above: creating a matrix using only rows that have a water column depth of 0

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
#above: filter(dataset, whatever you want to filter) - makes same thing as matrix filtering
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)
#above: say we consider every depth less than 0.25 to be 'surface level data' - filtering for rows w these values.
...
#above you filter for lakes you DONT want
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#cleaner way of selecting multiple conditions to be met
...
#day num is day of the year, so just look at numbers corresponding to months starting/ending
#first day of june is 152 if we look in the original dataframe, and last day of oct is 304 - so want all nums bw these
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305)
#comma is interpreted as 'combination of both conditions'
...
#new column appears as farthest to the right - what if we want month next to other date-related columns? (below)
...
#interval creates an interval object with specified start and end dates
...
#know that first row is first observed date and last row is last observed date
#first and last functions are in dplyr - better approach to use when using interval ()
```
...
#the above generates the same dataset we got to in a more long-winded way before!  
```
...
#every time you create a tidy, processed dataset you should save it somewhere outside of R (as a .csv preferably) for your records
#use write.csv to make a new file and save it in a folder with processed data
#this is the end of our wrangling process, and what we do next could be visualization or answering questions that we want to refer to the new data for
#also smart to save all the code as a 'wrangling code' file and then have a separate 'analysis/exploration code' file - helps w having cleaner scripts
```
