...
editor_options: 
  ...
  #asking output to go to console bc we will have a lot of output from the models in this. want plots to show up and outputs to go to console so that's why we changed chunk output type option
---
...
#y = cont. response var. x = explan var. a + B*x is straight line. + error term. a+Bx is deterministic, error term is distance from true value of y to straight line.
...
#no relationship bw vars means a and B = 0.
...
#regressing y based on one single variable, x.
...
#funciton lm = linear model
#y = irradiance (what to predict), x = depth (what you're predicting based on)
...
#lets you avoid using $ to call on specific columns
#residuals are the first thing in the report of the analysis
#coefficient estimates are the second: intercept and slope of regression line (x variable estimate). get standard errors for these estimates. p value here is less than 0 so our coefficients are statistically significant.
#3rd is significance codes: want standard error to be as small as possible. degrees of freedom are based on num of observations and variables. 
#R sqrd value tells us that x explains _% of variability in y variable - here, low at 30%
#p value for regression is less than 0 so we reject H0. 


# Correlation - related to variable relationship
...
>  Write out results and include numbers from analysis. 
...
#transforming screen into a grid of 2x2 plots - so you can see all 4 plots at the same time
...
#not a linear relationship - so we try log-transforming
#this plot still includes our outlier IDed in above 4 plots
```
...
#to take the log of irradiance values, have to get rid of 0s (can't take log of 0). to get rid of outlier, we remove values above 5k

#we see now that R sqrd increased after log-transforming the data
#and p value is still <0.05 (standard alpha)
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(irradiance.regression2)
par(mfrow = c(1,1))
#residuals plots look better than before
...
#to make scatterplot here, we use scale_y_log10 to put irradiance on log scale
...
print(irradiancebydepth2)
#se = FALSE removes grey confidence interval surrounding lines 
...
``` {r, fig.height = 3, fig.width = 4}
#now we have >1 x to explain variable y
#looking at Paul Lake specifically (so we subset data) - predicting total P from depth and DO data
...
#depth looks to positively impact P, and DO seems to negatively impact P

#visualizing the regression: scatterplot with DO on x, P on y, and color based on depth
#have to subset again just for Paul Lake
TPplot <- ggplot(subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                 aes(x = dissolvedOxygen, y = tp_ug, color = depth)) +
  geom_point() +
  xlim(0, 20)
print(TPplot)
#can see some correlation bw P and DO, as well as some impact from depth

#residuals plots:
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(TPregression)
#red line in plot 1 isn't flat, normal qq plot isn't bad but has some deviations, scale-location isn't flat, residuals v leverage indicates some outliers
par(mfrow = c(1,1))

#want to try to improve model below by looking for more relevant variables
```
...
#looking at correlation of coefficients can help us view which variables are relevant to model
library(corrplot)
#creating new dataframe with only columns of interest and omitting NAs
PeterPaulnutrients <- 
  PeterPaul.chem.nutrients %>%
  select(tn_ug:po4) %>%
  na.omit()
#create correlation matrix: call cor fxn to new dataframe from above. first line below just gives number. second line shows matrix. the ovals are scatterplots - thinner plot means closer to straight line so strong correlation. third line shows upper diagonal half only and gives numerical values of correlations as well as ovals. 
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse")
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
#from these, it seems like TN and NH34 might be good to add
#BUT don't want to overfit your model by adding more vars than you need - ideal model balances simplicuty and model power. Use AIC to check for this.
```
...
```{r}
#starting from original data, filtering for Paul and removing NAs
Paul.naomit <- PeterPaul.chem.nutrients %>%
  filter(lakename == "Paul Lake") %>%
  na.omit()
#considering all variables in the dataset in new model:
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
#AIC = 353.95 - want to minimize it by removing variables form the full model above. can see that if you remove po4, AIC will lower. same with if you then remove depth.removing add'l variables will increase AIC score.
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)
#here R sqrd actually decreases, but ideally you would be able to improve your regression summary values by using AIC and correlation matrix.
