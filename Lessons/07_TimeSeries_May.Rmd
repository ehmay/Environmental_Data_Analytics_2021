...
#having a big gap in dataset in time series is a big challenge - may lead us to split up data. 
...
#plotting discharge over time in cubic ft/second
#large gap from 1971 to 1985, which is a huge issue. we need a continuous record. so to work around: split data into early years and late years and run separate analyses
```
...
EnoDischarge.late <- EnoDischarge.complete %>%
  filter(datetime > as.Date("1985-01-01"))
#splitting up data to remedy issue of massive data gap in the middle of original data
```
...
#instead of $discharge.mean, could also say [8] bc it is the 8th column
#here we are getting discharge (from the 8th column), starting at Janurary 1985, and getting info daily
#time series objects =/= data frames! they are just value objects.
```
...
#1st plot = observed data
#2nd plot = seasonal component
#3rd plot = trend componnet
#4th plot = random/remainder (E) component
#this is not a monotonic trend bc it both increases and decreases on plot 3

# We can extract the components from plots above and turn them into data frames
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3])
#when you sum columns of seasonal, trend, and remainder data, you get the observed data
#time series object automatically specifies the end date so we don't have to
...
#adding 2 new columns: one with observed data (taken from original dataframe) and one from date object (also from original dataframe)
#so now our components dataframe has observed data, seasonal data, trend data, remainder data, and date data

# Visualize how the trend maps onto the data
#give ggplot components dataframe, add line for observed data, add line for trend component (diff color from observed line), add line at intercept y=0 so we can visualize that our trend component can be < 0 even though our observation data is never < 0
...
#same as above but for seasonal component rather than trend. here we see seasonal component goes below our y=0 intercept line.
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
...
### Monotonic trend analysis (only 1 direction of trend present in time series)
...
#we have one NA in our data - so we are only missing 1 datapoint, so can interpolate rather than splitting the dataset
...
#there are sometimes fxns that have same name but from diff package, so above, we specify name of package, zoo, to make sure that R uses the correct function. when we do this, we don't have to add that package (zoo) to our library earlier in the code bc this calls it into play. 
#na.approx() is a fxn to clean the data
...
#approximating the dataset - the red segment of line in this plot is filling in for the NA, doing interpolation to fill in the gap. can't just remove NA in time series, have to replace them. UNLESS NA is at very beginning or end. 
```
...
#calling ts function on cleaned wind speed data but instead of manually providing first year, month of observation, want this to be automatic so that it's reproducible for other datasets. so below we define first month and year
...
wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   ...
                   frequency=12) 
#freq = 12 bc working with monthly data
#so now we have wind time series

#decompose the above time series we made
wind_data_decomp <- stl(wind_data_ts,s.window = "periodic")
#much easier to spot seasonal component now, so we know we're dealing w seasonal data:
plot(wind_data_decomp)
#trend moves up and down but overall looks like it's decreasing. but need to run stats test(s) to prove this rather than just eye guessing.

#bc we now have confirmed this is seasonal data, we use seasonal Mann-Kendall:
#from package kendall
# Run SMK test
...
#hypothesis for M-K: H0 = data is stationary. Ha: data is not stationary. So this data is not stationary - we have a trend in it!

#can do the same thing as above but from pacakge trend and diff fxn:
wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
wind_data_trend2
summary(wind_data_trend2)
#this test gives us results for tau of each year. also gives us the value of S. so we can see the strength of S across different months (seasons). 
#S is negative in the above results so overall have decreasing trend!

#Visualization: plot wind data vs time and run a linear regression over time series plot:
wind_data_plot <-
...
#confirmed the decreasing trend via trendline

```

What would we conclude based on these findings? 

> Answer: Decreasing trend in wind speed over time, with inner seasonal variations. 
