...
```{r}
high.temps.F <- c(48, 52, 48, 50, 50, 50, 48, 39, 39, 36)
low.temps.F <- c(25, 39, 28, 30, 27, 34, 30, 25, 19, 19)
...
high.temps.C <- F.to.C(high.temps.F)

low.temps.C <- F.to.C(low.temps.F)
...
```{r}
durm.temps <- data.frame(high.temps.F, low.temps.F, high.temps.C, low.temps.C)
colnames(durm.temps) <- c("High Temperatures (F)", "Low Temperatures (F)", 
...
```{r}
summary(durm.temps)
sd(durm.temps$`High Temperatures (F)`)
sd(durm.temps$`Low Temperatures (F)`)
sd(durm.temps$`High Temperatures (C)`)
sd(durm.temps$`Low Temperatures (C)`)
#can do summary on whole dataframe but sd has to be done per column
```
...
#Ex1
str_today <- "2021-feb-3" #this is just a character string rn - not a date object
...
```{r DateExercise2}
#century issue
str_past <- "55-feb-3" #we are talking about 1955
date_obj_past <- ymd(str_past)
date_obj_past #this defaults to 2055
