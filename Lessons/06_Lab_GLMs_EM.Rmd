...
> Answer: Null: AQI is greater than or equal to 50. Alternative: AQI is less than 50. H0: mu >= 50; Ha: mu < 50
...
#selecting a subsample of the data by selecting a few rows on the dataset:
EPAair.subsample <- sample_n(EPAair, 5000)

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone))
#so we reject H0 - not normally distributed.
#plot histogram to visualize the data:
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
#histo is a little skewed - more data on right tail than left, but also looks bimodal rather than the normal dist's unimodal distr
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)
#quantiles are veering off the straight line in second half

#run t test to check our hypothesis. alternative can be less, greater, or two sided. doing t test bc we don't have normal distr.
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample
#reject H0 - mean is not 50 - it is less than 50.
...
#ratio of variances is not = to 1.
...
ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly()
#looks like 2018 levels were < 2019 levels

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year)
O3.twosample
O3.twosample$p.value
#means are statistically different based on p value
...
ggplot(EPAair, aes(x = Ozone, apha=Year)) +
         geom_density()
...
O.PM <- lm(data=EPAair, PM2.5~Ozone)
summary(O.PM)
#p = 2.2e-16
#R sqrd = 13
...
air.plot <- ggplot(EPAair, aes(x=Ozone, y=PM2.5)) +
  geom_smooth(method="lm") +
  stat_regline_equation(formula=EPAair$PM2.5~EPAair$Ozone) +
  geom_point() 
#how to add regression equation??
...
air2 <- EPAair %>%
  select(Month, Year, SITE_LATITUDE, SITE_LONGITUDE) %>%
  na.omit()
EPAcorr <- cor(air2)
...
OzonebyALL.regress <- lm(data=EPAair.subset, PM2.5~Ozone + Month + SITE_LATITUDE + SITE_LONGITUDE)
summary(OzonebyALL.regress)
