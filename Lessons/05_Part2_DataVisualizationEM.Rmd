...
O3plot1 <- ggplot(EPAair) +
  ...
  theme_gray()
print(O3plot1)
#this one is the default ggplot theme

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()
print(O3plot2)
#bw is classic dark and light them - background is white and axis lines are black

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()
print(O3plot3)
#light = white background, grey axis lines

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()
print(O3plot4)
#no gridlines, white background and black axis lines
```
...
#using classic theme but increasing font size for labels and make axes text black rather than the usual grey
#above we are just making these specifications into an object that we can refer to when making below plots
...
theme_set(mytheme)
#use this if you're using the same theme in all subsequent plots ^
...
#geom_point will add your data points
#alpha determines transparency
#geom_text adds text to the plot
# %b is the abbreviated month format (e.g. Jan.)
...
print(NvsP2)
#labs() lets us change labels of legend
...
#cowplot puts multiple plots on one page
#rel_heights adjusts space for first then second plot (relative heights so that if one plot needs more space than the other they don't look weird together)
```
