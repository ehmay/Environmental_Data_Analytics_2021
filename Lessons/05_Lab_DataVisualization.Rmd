...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #express for this label bc mix of letters and symbols - for Greek symbols you just write out their name w/o ''
print(Plot_text)
#paste glues things together - in this case string, greek letter, string, numbers. 
...
ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color=depth), dotsize = 0.5) + geom_point() + scale_color_viridis(option = "magma", direction = -1)
...
Neonics$Endpoint <- reorder(Neonics$Endpoint, Neonics$Endpoint, FUN = length) #reordering by length 
#above: reorder(factor you want to reorder, factor that defines how we reorder)

Neonics.plot <- ggplot(Neonics, aes(x=Endpoint)) +
  geom_bar(fill="#7fcdbb", color="black", size=0.24) +
  scale_x_discrete(limits = rev(levels(Neonics$Endpoint))) +
  labs(y="# of pubs") + theme(axis.text.x = element_text(angle=45, hjust=1))
print(Neonics.plot)
#used rev so it goes from high to low counts rather than the usual low to high
#changed x axis angling to fit all the labels
#changed bar color via fill
