...
#Create a map
mapview(huc8_sf, col.regions = "purple", layer.name = "NC River Subbasins") + mapview(huc8_sf_un, col.regions = "orange", layer.name = "Upper Neuse Subbasin")
...
huc8_wind <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv") %>%
...
uniques(huc8_sf$DWG_Basin)


#Summarize on DWQ Basin value
huc2_sf <- huc8_sf %>%
  group_by(DWQ_Basin) %>%
  summarize(ACRES = sum(ACRES), SQ_MILE = sum(SQ_MILES))
