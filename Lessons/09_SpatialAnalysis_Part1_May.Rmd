...
#group dataset based on location then summarize mean and max concentrations^
...
#imagine the plot you're viewing overtop the shape of NC
#so the distances bw points are distances geographically IRL
#color is the value of mean concentration
#coord equal makes x and y axes on similar scales
```
...
#mapview operates kind of like ggplot
...
myMap = mapview(EPAair_PM_avg,
         ...
         crs = 4269, 
         grid = FALSE)
#^this is like setting mytheme

#Show the map
myMap

#this is the same plot (in terms of point) as above but on an actual map, which you can zoom in on bc map is connected to web. if you click on a point on the map you see its attributes (geometry and other attributes)

#go to stacked layers on the left side of map to change map background layer
...
#leaflet is similar, a bit more tidy than mapview. also more complex but gives you more capability. add tiles for what map will look like and can change what data points look like
...
#can add popups to show things like site names on the map
#made radius of points to be based on mean concentration
...
#st_as_af is similar to as.dataframe

#have to specify geometric coordinates
...
sf_EPAair_PM_avg <- st_as_sf(EPAair_PM_avg,
                             ...
                             crs=4326)
#we've lost a variable from original df --> lat and long are gone, have been combined into new 'geometry' column

#Do you see a new column name? --> geometry col                
colnames(sf_EPAair_PM_avg)

#What is the class of the values in the geometry column?
class(sf_EPAair_PM_avg$geometry)
#2 classes ^ : general object and also a special case of this general object. gen obj = sf, specific geometric type = point
...
#very simply plot but can see the shape of NC in points

#Plot everything
plot(sf_EPAair_PM_avg)
#each plot uses diff legend colors to depict diff information
...
#now that we have a defined geometry column, we can use it in ggplot. sf package loads a geo component into ggplot.
...
#ggmpa allows you to bring in map backdrops with ggplot but there are licensing issues rn so using it is unstable 

#in aesthetics we don't have to specify x and y columns bc it knows to do this via geometry column (we're plotting like it's on a map in space), we just define colors based on what info at each coordinate we want to show

#Plot everything -- with MapView
mapview(sf_EPAair_PM_avg)
#only need to specify spatial dataframe for mapview to plot it

#View multiple attributes with MapView
mapview(sf_EPAair_PM_avg, 
        zcol='meanPM', 
        cex='maxPM')
#making mapview map prettier ^

#Plot with leaflet using markers
leaflet(sf_EPAair_PM_avg) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addMarkers(popup = ~as.character(maxPM))
#popup = you click to get info rather than hovering
...
#scaling red values based around mean PM values^
...
#better way to save it is via export ^ should save to output folder.
...
#comes out as an oval on the map bc of coordinate reference systems. if we put the buffer on a globe, it would be a perfect circle, but we get distortion bc of flat map. 
```
...
#our dataset has a defined CRS, which we defined when we first created it in 2.4.1 (used crs = 4326 [WGS 84 CRS] in the code)
#moving CRS to UTM one that we looked up and that matches with our location in NC
sf_EPAair_PM_avg_UTM <- st_transform(sf_EPAair_PM_avg, crs=26917)
# see that now our new coordinates are in METERS, referenced to origin of UTM 17

#Rebuffer the Durham Armory site. Is it now circular? 
#we buffer in meters now bc CRS coords are in meters
...
#now we get a perfect circle on the map bc we are showing the buffer on a flat surface. before, we were showing it on spheroid/ellispoid surface (rounded Earth)
```
...
#subsetting the above dataframe to get one point as new df
...
#this type of geometry object is a point

#Show the point
mapview(sf_DurhamArmory)
#map of our single point
#can make geometries from a table like above or can do it from a single point, like we will for this point below:
```
...
#Examine this item
class(grainger_sfg)
#spatial feature geometry, which does NOT include coordinate reference system^

#Convert to a sfc object, adding a coordinate reference system
grainger_sfc <- st_sfc(grainger_sfg, crs=4326)
#converting to simple feature class/column bc it will include CRS so we know where we are in space (geometry + reference info)
...
mapview(sf_DurhamArmory, 
  ...
  mapview(grainger_sfc, 
          ...
          alpha.regions=1)
#created a map with both points we created above ^
```
...
#Durham Armory (above) is in meters using UTM bc we converted it earlier, but Grainger Hall is still in lat long. need coordinates to be the same scale/type if we measure distance bw them.

#Transform the Grainger hall SFC to UTM Zone 17n
grainger_sfc_utm <-  st_transform(grainger_sfc, crs=st_crs(sf_DurhamArmory))
#rather than referring directly to the UTM CRS, can just refer to the same CRS as Durham Armory^

#Check the EPSG of our transformed object; it should equal 26917
st_crs(grainger_sfc_utm)$epsg
#^lets us ask what the CRS is

#Now, try computing distance again
the_distance <- st_distance(x = sf_DurhamArmory,
                            y = grainger_sfc_utm,
                            by_element = TRUE)
#note that when we view the_distance it gives us the units as well - meters
