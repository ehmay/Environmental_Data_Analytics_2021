...
#. above is replaced by the absolute path, which is the working directory full path
...
#character - in csv files dates are always brought in as character columnds
...
ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()
#tweaking the above to not include rows with NA:
...
#non-normal distr - skewed toward 0
...
#this one is looking at a diff value - gage heigh mean, whereas above are all discharge mean
```
...
#here on the y axis remember we are looking at COUNTS/frequency of values on x axis
#looking at mean, min, and max for gage height
#lty = line type, 2 = dashed
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean, color = gage.height.mean.approval), bins = 50) +
  scale_x_continuous(limits = c(0, 10)) +
  theme(legend.position = "top")
#we are adding another aesthetics category here: color
#part of code with color = __ <- R looks at unique values in __ and that's where it gets info for the legend and lines - colors each line by the unique values that exist - here, just A and P records
```
...
#group and cut_width determine how we 'slice' the data, similar to a histo
#instead of showing how many records fall into each slice, it shows the distribution of values within each slice 
...
#each point is from a row in dataframe

ggplot(USGS.flow.data) +
  geom_point(aes(x = datetime, y = discharge.mean))
#datetime is kind of a hybrid bw categorical and continuous data
#showing mean discharge at diff times
#gap is a time period when we have no data for this site
```
